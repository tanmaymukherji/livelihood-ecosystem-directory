...
WORKBOOK_PATHS = [
    Path(r"C:\Users\tmukh\OneDrive\Desktop\Livelihood Network Database\V1 - Rural_Volunteer_Networks_India.xlsx"),
    Path(r"C:\Users\tmukh\OneDrive\Desktop\Livelihood Network Database\V2 - Rural_Volunteers_India_Consolidated_500.xlsx"),
]
OUTPUT_SQL_PATH = PROJECT_ROOT / "supabase" / "migrations" / "20260514143000_import_consolidated_volunteers.sql"
OUTPUT_REPORT_PATH = PROJECT_ROOT / "scripts" / "consolidated_volunteer_import_report.json"
CONFIG_PATH = PROJECT_ROOT / "config.js"
PUBLIC_VIEW = "ecosystem_directory_entities"
USER_AGENT = "Livelihood Ecosystem Directory Consolidated Volunteer Import/1.0"
SSL_CONTEXT = ssl._create_unverified_context()

SOURCE_LABEL = "Consolidated volunteer workbooks"
CREATED_BY_NAME = "Consolidated volunteer import"
...
def fetch_existing_rows(base_url, anon_key):
    ...
    url = (
        ...
        f"&entity_type_slug=eq.volunteer&limit=5000"
    )
    headers = {
        "apikey": anon_key,
...
def normalize_url(value):
    ...
    if not text:
        return None
    text = re.sub(r"^(FB|LI|TW|IG)\s*:\s*", "", text, flags=re.I)
    if text.startswith("www."):
        ...
        text = "https://" + text
    elif text.startswith("linkedin.com") or text.startswith("facebook.com") or text.startswith("instagram.com") or text.startswith("twitter.com") or text.startswith("x.com"):
        text = "https://" + text
...
def parse_social_media(value, fallback_website=None):
    text = clean_text(value)
    if not text:
        return {}
    social = {}
    ...
    for part in parts:
        ...
        if part.startswith("/company/") and fallback_website:
            url = f"https://www.linkedin.com{part}"
        if part.startswith("/") and "facebook" in lower:
            url = "https://www.facebook.com" + part
        if part.startswith("@"):
            handle = part.lstrip("@")
            if "instagram" in lower or "ig:" in lower:
                url = f"https://www.instagram.com/{handle}"
            elif "twitter" in lower or "tw:" in lower:
                url = f"https://twitter.com/{handle}"
            elif "facebook" in lower or "fb:" in lower:
                url = f"https://www.facebook.com/{handle}"
        if "li:" in lower and "/company/" in lower:
            url = f"https://www.linkedin.com{part.split(':',1)[1].strip()}"
        elif "fb:" in lower and "@" in part:
            url = f"https://www.facebook.com/{part.split('@',1)[1].strip()}"
        elif "ig:" in lower and "@" in part:
            url = f"https://www.instagram.com/{part.split('@',1)[1].strip()}"
        elif "tw:" in lower and "@" in part:
            url = f"https://twitter.com/{part.split('@',1)[1].strip()}"
        if not url:
            continue
        lower_url = url.lower()
        if "linkedin.com" in lower_url:
            ...
            social["other"] = dedupe((social.get("other") or []) + [url])
    return social
...
def normalize_availability_type(value):
    ...
    if "flex" in lower or "virtual" in lower or "part-time" in lower or "part time" in lower or "full-time" in lower:
        return "Flexible"
    return "Flexible"
...
def infer_state(place, geography_items):
    haystack = ", ".join([clean_text(place) or "", ", ".join(geography_items)]).lower()
    for state in STATE_NAMES:
        if state.lower() in haystack:
            return state
    if "pan-india" in haystack or "all india" in haystack:
        return "India"
    return None
...
def geocode_location(place, state, geography_items, cache):
    ...
    generic = {"india", "pan-india", "all india", "rural india", "virtual"}
...
def build_entity_uid(name):
    ...
    return f"volunteer-{slugify(name)}-{digest}"
...
def load_rows(path: Path):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if not name or name.lower() == "name":
            continue
        website = normalize_url(row[4])
        rows.append({
            "name": name,
            "place": clean_text(row[1]),
            "contact_email": extract_email(row[2]),
            "contact_email_note": clean_text(row[2]),
            "contact_phone": extract_phone(row[3]),
            "contact_phone_note": clean_text(row[3]),
            "website_url": website,
            "social_media": parse_social_media(row[5], website),
            "skills": split_list(row[6]),
            "cause_areas": split_list(row[7]),
            "languages_spoken": split_list(row[8]),
            "preferred_geography": normalize_geography_tokens(row[1], row[9]),
            "past_experience": clean_text(row[10]),
            "availability_type": normalize_availability_type(row[11]),
            "availability_hours": clean_text(row[11]),
...
def merge_raw_rows(base, incoming):
    ...
    merged["contact_email_note"] = pick_richer(merged.get("contact_email_note"), incoming.get("contact_email_note"))
    merged["contact_phone"] = pick_richer(merged.get("contact_phone"), incoming.get("contact_phone"))
    merged["contact_phone_note"] = pick_richer(merged.get("contact_phone_note"), incoming.get("contact_phone_note"))
    ...
    merged["skills"] = merge_arrays(merged.get("skills"), incoming.get("skills"))
    merged["cause_areas"] = merge_arrays(merged.get("cause_areas"), incoming.get("cause_areas"))
    merged["languages_spoken"] = merge_arrays(merged.get("languages_spoken"), incoming.get("languages_spoken"))
    merged["preferred_geography"] = merge_arrays(merged.get("preferred_geography"), incoming.get("preferred_geography"))
    merged["past_experience"] = pick_richer(merged.get("past_experience"), incoming.get("past_experience"))
    merged["availability_type"] = pick_richer(merged.get("availability_type"), incoming.get("availability_type"))
    merged["availability_hours"] = pick_richer(merged.get("availability_hours"), incoming.get("availability_hours"))
...
def build_summary(raw):
    skills = ", ".join(raw.get("skills")[:3]) if raw.get("skills") else "community volunteering"
    causes = ", ".join(raw.get("cause_areas")[:3]) if raw.get("cause_areas") else "rural causes"
    geography = ", ".join(raw.get("preferred_geography")[:3]) if raw.get("preferred_geography") else "India"
    return f"{raw['name']} can support {causes} through {skills} across {geography}."


def build_description(raw):
    parts = []
    if raw.get("past_experience"):
        parts.append(raw["past_experience"])
    if raw.get("contact_email_note") and raw.get("contact_email_note") != raw.get("contact_email"):
        parts.append(f"Email access note: {raw['contact_email_note']}")
    if raw.get("contact_phone_note") and raw.get("contact_phone_note") != raw.get("contact_phone"):
        parts.append(f"Phone access note: {raw['contact_phone_note']}")
...
def build_record(raw, existing, geocode_cache):
    state = infer_state(raw.get("place"), raw.get("preferred_geography") or [])
    location_label = raw.get("place") or (", ".join(raw.get("preferred_geography") or []) or "India")
    lat, lng = geocode_location(raw.get("place"), state, raw.get("preferred_geography") or [], geocode_cache)
    type_specific = {
        ...
        "preferred_geography": raw.get("preferred_geography") or [],
        "past_experience": raw.get("past_experience"),
    }
    social_media = dict(raw.get("social_media") or {})
    if raw.get("website_url") and not social_media.get("website"):
        social_media["website"] = raw["website_url"]
    ...
    record = {
        ...
        "tags": merge_arrays(
            ...
            raw.get("skills") or [],
            raw.get("cause_areas") or [],
            raw.get("preferred_geography") or [],
            [SOURCE_LABEL],
        ),
        ...
        "source_url": raw.get("website_url"),
        ...
        "admin_notes": f"Imported from consolidated volunteer workbooks | Source files: {', '.join(raw.get('source_files') or [])}",
...
def build_insert_sql(records):
    return """insert into public.volunteer_entities (
  entity_uid,
  entity_name,
  summary,
  description,
...
def main():
    ...
    report = {
        ...
        "existing_live_volunteer_count": len(existing_rows),
        "inserted": 0,
        "merged": 0,
        "with_website": 0,
        "with_social_media": 0,
    ...
    print(json.dumps({
        ...
        "existing_live_volunteer_count": report["existing_live_volunteer_count"],
